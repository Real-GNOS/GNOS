...
typedef struct {
     ...
     * here before every create; both default to root at mount time. */
...
/* Change owner and/or group.  (uint32_t)-1 leaves a component unchanged. */
int ext2_chown(ext2_fs_t *fs, ext2_dirent_t *ent, uint32_t uid, uint32_t gid);

/* Owner stamped onto inodes created from now on (see ext2_create).  The VFS
 * sets this from the calling process's effective ids before each create. */
