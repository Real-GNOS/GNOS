 ...
 *
 * This is the one header all of the DRM core shares: the device, the driver
 * that backs it, the per-open file, the KMS objects (CRTC, plane, encoder,
 * connector, framebuffer, property, mode), the buffers (GEM), and the
 * transaction object an atomic commit is built in.
 *
 * Anything that comes from outside the DRM core -- a VFS node, a bus device,
 * a process -- is held as an opaque pointer on purpose.  The core does not
 * need to know what those are, and keeping them opaque is what stops it from
 * growing dependencies on the rest of the kernel.
 *
 * These types are the contract between the modules in this directory, so the
 * field order matters: each module compiles against the same layout.
 */
...
/*
 * An event waiting to be delivered: a vblank or a completed page flip, for a
 * particular frame, to a particular client.  While it is queued it holds a
 * reference to the file it will be sent to, which is what keeps that file
...
struct drm_pending_vblank_event {
    ...
    unsigned int            pipe;
    uint64_t                sequence;   /* frame it fires on */
    bool                    vblank_ref; /* holds a vblank reference */
    bool                    file_ref;   /* holds a reference on file_priv */
    struct drm_event_vblank event;
    void (*destroy)(struct drm_pending_vblank_event *e);
    struct drm_pending_vblank_event *next; /* in the CRTC's event queue */
};

/* Per-CRTC frame counter and everything that depends on it. */
...
/* Why a proposed mode was rejected, or that it was accepted. */
...
/* What a plane is for: the desktop, the pointer, or an extra layer. */
enum drm_plane_type {
...
/* The properties attached to one object: parallel id and value arrays. */
...
/* The common prefix of every KMS object: its id, its kind, its refcount. */
struct drm_mode_object {
...
/* An opaque byte string with an id: gamma tables, mode lists, EDID. */
struct drm_property_blob {
    struct drm_mode_object base;
    void                  *data;
    size_t                 length;
...
/*
 * One video mode.  The plain fields are what the monitor reports; the crtc_*
 * ones are what the hardware is actually programmed with, which differs once
 * an encoder has had its say.
 */
struct drm_display_mode {
    struct drm_mode_object base;
    char                   name[DRM_DISPLAY_MODE_LEN];
    int                    connector_count; /* connectors listing this mode */
...
/* ------------------------------------------------- driver callbacks, per object */

/* What a driver does for its CRTC: set a mode, flip, move the cursor, and be
 * told when it is switched on or off. */
...
/* The proposed (or committed) state of one CRTC.  The *_changed flags are how
 * an atomic commit records what it disturbed. */
struct drm_crtc_state {
    struct drm_crtc                 *crtc;
    bool                             active;
    bool                             enable;
...
/* Where a plane reads from (src, 16.16 fixed point) and writes to (dst). */
struct drm_plane_state {
...
/* One plane's slot in a transaction: what it was, what it becomes. */
...
/* One CRTC's slot in a transaction. */
...
/*
 * A whole proposed configuration.  CRTC and plane slots are arrays indexed by
 * the object's position on the device's list; connectors grow on demand,
 * because a commit usually names only one or two of them.
 */
struct drm_atomic_state {
    struct drm_device             *dev;
    struct drm_file               *file_priv;
...
/* Everything the device owns, and the limits on what it will accept. */
struct drm_mode_config {
    ...
    /* The standard properties, created once and attached to each object. */
    ...
    uint64_t     commit_queue_next; /* tickets handed out */
    uint64_t     commit_queue_done; /* tickets finished */
};
...
/* Feature bit shims, kept local rather than added to common.h. */
#define BIT0_ (1U << 0)
#define BIT1_ (1U << 1)
#define BIT2_ (1U << 2)
#define BIT3_ (1U << 3)
...
/* One command: its number, its handler, and who may call it. */
...
/* One queued event on its way to a client. */
struct drm_event_node {
    struct drm_event      *event;
    struct drm_event_node *next;
};

/* A GEM handle: a per-file name for a buffer. */
...
struct drm_file {
    ...
    bool                   event_closing;
    uint32_t               event_refs; /* events armed but not yet delivered */
};
