...
/*
 * A handful of calls are never restarted, no matter what SA_RESTART says.
 * Linux marks them -ERESTARTNOHAND; the common thread is that their argument
 * is a *deadline* or a one-shot wait, so running them again would either wait
 * a second full period or -- worse -- wait forever for an event that already
 * happened while the handler was running.
 *
 * sigsuspend(2) is the one that bites hardest.  Its whole contract is "sleep
 * until a signal arrives, then return EINTR", so restarting it after the
 * handler puts the process straight back to sleep and the caller never gets
 * to look at what woke it.  timeout(1) is built exactly that way --
 *
 *     while (waitpid (child, &status, WNOHANG) == 0)
 *       sigsuspend (&unblocked);
 *
 * -- and its SIGCHLD handler carries SA_RESTART, so a restarting sigsuspend
 * means the waitpid() at the top of the loop never runs again and
 * `timeout 1 sleep 5` hangs the boot.
 */
static int never_restarts(int nr)
{
    switch (nr) {
    case SYS_sigsuspend:
    case SYS_poll:
...
static void maybe_restart_syscall(proc_t *p, regs_t *r, int sig)
{
    ...
    if (!(p->sigact[sig].flags & SA_RESTART))
        return;
    if (never_restarts(p->syscall_nr))
        return;
