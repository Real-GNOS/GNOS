...
/* ---- power management (PM1a) ------------------------------------------- */

/* The SLP_TYP value the firmware's _S5 object says puts the machine to
 * sleep.  It is AML, and there is no interpreter here, but the value itself
 * is structurally trivial to find: an AML name is stored as its literal
 * bytes, and the _S5 package's first element is PM1a_CNT.SLP_TYP, an
 * integer constant.  QEMU's own DSDT declares _S5 = {0,0,0,0}, so the
 * scalable-scan for BytePrefix etc. also happens to be the QEMU answer. */
static uint16_t s5_slp_typ(void)
{
    const acpi_sdt_t *d = acpi_find("DSDT");
    if (!d)
        return 0;

    const uint8_t *end = (const uint8_t *)d + d->length;
    for (const uint8_t *p = (const uint8_t *)d; p + 6 <= end; p++) {
...
/* The SCI (System Control Interrupt) arrives on GSI 9.  QEMU raises it the
 * moment the power button event is armed AND the button is pressed; the
 * status bit stays set until we clear it, so the interrupt level stays
 * high and would re-fire forever -- clear first, then commit to S5. */
static void pm1a_sci(regs_t *r)
{
    (void)r;
...
{
    g_slp_typ = s5_slp_typ();

    if (!g_pm1a_evt) {
        dbg_puts("ACPI: no PM1a event block; power button unavailable\r\n");
        return;
    }
...
{
    if (!g_pm1a_cnt) {
        dbg_puts("ACPI: no PM1a control block; cannot power off\r\n");
        return;
    }

    /* SLP_TYP(S5) | SLP_EN in one write, the way QEMU and every chipset
     * expect it: the firmware takes it from there.  If the write is
     * somehow ignored, park instead of falling back into a running
     * system -- a half-sleep is worse than a hang. */
    dbg_puts("ACPI: writing SLP_EN (S5)\r\n");
    outw(g_pm1a_cnt, (uint16_t)(g_slp_typ | PM1_SLP_EN));
    for (;;)
